...
whisperCreateButton.addEventListener('click', (event) => {
  ...
  if (message) {
    createWhisper(message).then(refreshAllUI);
  }
});
...
const fetchAllWhispers = () =>
  fetch('http://localhost:3000/api/v1/whisper').then((response) =>
    response.json()
  );
const deleteWhisper = (id) =>
  fetch(`http://localhost:3000/api/v1/whisper/${id}`, { method: 'DELETE' });
